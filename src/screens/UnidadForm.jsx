...
export default function UnidadForm({ batch, modelosDisponibles, tecnico, onBack, onGuardada }) {
  ...
  function siguiente() {
    ...
    if (paso === TOTAL_PASOS) {
      ...
      if (requiereNewSn) {
        if (!newSn.trim()) {
          setError("Esta decisión requiere el nuevo número de serie.");
          return;
        }
        if (newSn.length !== LARGO_SERIAL) {
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        {paso === 2 && (
          <>
            ...
            <input
              value={oldSn}
              onChange={(e) => setOldSn(limpiarSerial(e.target.value))}
              disabled={oldSnNa}
              placeholder={`O escribe los ${LARGO_SERIAL} dígitos`}
              inputMode="numeric"
          ...
          <>
            ...
            {requiereNewSn && (
              <>
                ...
                <input
                  value={newSn}
                  onChange={(e) => setNewSn(limpiarSerial(e.target.value))}
                  placeholder={`O escribe los ${LARGO_SERIAL} dígitos`}
                  inputMode="numeric"
                  maxLength={LARGO_SERIAL}
                  style={inputStyle}
